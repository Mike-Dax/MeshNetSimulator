...
function createSim(graph) {
  ...
  function updateSimStatistics() {
    ...
    var routingEfficiencyPercent =
      100 * routesEfficiencySum / routesEfficiencyCount
    ...
    $$('packets_broadcast').nodeValue = withPercent(
      packets_broadcast,
      packets_unicast + packets_broadcast
    )
    $$('packets_unicast').nodeValue = withPercent(
      packets_unicast,
      packets_unicast + packets_broadcast
    )
    $$('packets_per_node').nodeValue = intNodes.length
      ? ((packets_unicast + packets_broadcast) / intNodes.length).toFixed(2)
      : '-'

    $$('routes_count').nodeValue = routesCount
    $$('routes_packets_send').nodeValue = routesSend
    $$('routes_packets_received').nodeValue = withPercent(
      routesReceived,
      routesSend
    )
    $$('routes_packets_transit').nodeValue = withPercent(
      routesTransit,
      routesSend
    )
    $$('routes_packets_lost').nodeValue = withPercent(
      routesSend - routesReceived - routesTransit,
      routesSend
    )
  ...
  function updateRoutesTable() {
    ...
    for (key in self.routes) {
      ...
      append(
        ...
        num(
          100 *
            (route.sendCount - route.receivedCount - route.transitCount) /
            route.sendCount,
          '%'
        )
      )
  ...
  function updateRoutesEfficiency() {
    ...
    for (var id in self.routes) {
      var route = self.routes[id]
      var sourceIntNode = intNodes.find(function(e) {
        return e.o.mac === route.sourceAddress
      })
      var targetIntNode = intNodes.find(function(e) {
        return e.o.mac === route.destinationAddress
      })
      if (sourceIntNode && targetIntNode) {
        ...
        */
        route.efficiency =
          shortestDistance * route.receivedCount / route.receivedStepCount
      }
    }
  }
  ...
  self.run = function run(steps, deployPacketsEnabled) {
    ...
    for (var step = 0; step < steps; step += 1) {
      ...
      // Propagate packets waiting in links

      for (var i = 0; i < intLinks.length; i++) {
        var intLink = intLinks[i]

        // if we have packets that should be delivered this step
        if (intLink.o.inTransit[self.simStep]) {
          for (var j = 0; j < intLink.o.inTransit[self.simStep].length; j++) {
            var obj = intLink.o.inTransit[self.simStep][j]
            var packet = obj.packet
            var destination = obj.destination

            // Packet transmitted
            destination.o.incoming.push(packet)

            // update our stats
            if ('deployedAtStep' in packet) {
              if (packet.destinationAddress === destination.o.mac) {
                packetOnRouteReceived(packet)
              }
            }
          }

          delete intLink.o.inTransit[self.simStep]
        }
      }
          ...
          for (var j = 0; j < intLinks.length; j++) {
            ...
            if (isBroadcast || packet.receiverAddress === otherIntNode.o.mac) {
              ...
              if (intLink.o.transmit(packet, packetCount[intLink.index])) {
                // successfully going to cross the link. figure out
                var latency = intLink.o.getLatency(
                  packet,
                  packetCount[intLink.index]
                )
                // latency should never be 0
                var arrivalStep = self.simStep + latency

                if (!intLink.o.inTransit[arrivalStep]) {
                  intLink.o.inTransit[arrivalStep] = []
                }

                intLink.o.inTransit[arrivalStep].push({
                  packet: packet,
                  destination: otherIntNode
                })
              }
